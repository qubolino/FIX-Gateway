...
class MainThread(threading.Thread):
    def __init__(self, parent):
        ...
        self.count = 0
        self.sleep_time = 1.0/5 # 3 x .005 give +/-60Hz refresh rate
    ...
    def run(self):
        while True:
            try:
                ...
                data = bytearray(self.i2c_bus.read_i2c_block_data(0x04,0x00,4*3))
                # print(struct.unpack('f', data[:4]), struct.unpack('f', data[4:8]), struct.unpack('f', data[8:12]))
                ias = struct.unpack('f', data[8:12])[0]  # m/s
                ias *= 3.6
                if ias > 0:
                    self.parent.db_write("IAS", ias)
                else:
